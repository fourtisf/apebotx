#!/usr/bin/env node
/**
 * Register the ApeBotX Telegram bot: set the command webhook + the slash-command
 * menu. Run once after deploy (and whenever the URL/secret changes).
 *
 *   TELEGRAM_BOT_TOKEN=... \
 *   NEXT_PUBLIC_SITE_URL=https://apebotx.io \
 *   TELEGRAM_WEBHOOK_SECRET=some-random-string \
 *   node scripts/setup-telegram-bot.mjs
 */
const token = process.env.TELEGRAM_BOT_TOKEN;
const site =
  process.env.NEXT_PUBLIC_SITE_URL || process.env.SITE_URL || "https://apebotx.io";
...
const cmds = await post("setMyCommands", {
  commands: [
    { command: "start", description: "About ApeBotX" },
