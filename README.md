...
- `src/app/icon.svg` — favicon: the chevron mark, legible at 16 px.
- `public/social/x-avatar.{svg,png}` — X profile picture (400×400, circle-safe). The
  `.svg` is rebranded; regenerate the `.png` from it before launch.
- `public/social/x-banner.{svg,png}` — X header (1500×500). The `.svg` is rebranded;
  regenerate the `.png` from it before launch.
